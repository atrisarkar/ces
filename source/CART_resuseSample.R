...
# Initialization ##################################################################################
initData <- function(){
	cat("Please enter full address of the dataset (for example: C:\\Data\\Dataset.csv)", '\n')
	fileAddress <<- scan(file = "", what = " ", n = 1, quiet = TRUE)
	# fileAddress <- "E:\\CS886TEMP\\FULL\\Input\\Apache.csv"
	
	cat("Please enter address of output folder (for example: C:\\Output)", '\n')
...
initSamplingParams <- function(){
	...
	#samplingUpper <<- scan(file = "", what = integer(), n = 1, quiet = FALSE)
	 samplingUpper <<- ((obsCount %/%  samplingProgressionBase)*samplingProgressionBase) - samplingProgressionBase
	 print(samplingUpper)
}
...
analyseCART <- function()
{
	...
	# Main loop ###################################################################################
		for(seedIter in 1:seedRepetitions){
			crs$train <- NULL
					for(samplingIter in samplingVector){
							...
							crs$sample <- sample(nrow(crs$dataset), samplingProgressionBase)
							if(is.null(crs$train)){
								crs$train <- crs$sample
							} else {
								crs$train <- cbind(crs$train,crs$test)
								crs$test <- NULL
							}
							...
							crs$test <- sample(crs$train.test.diff, samplingProgressionBase)
						...
						#faultRate <- mean(colMeans(faultDataset))
						faultRate <- colMeans(faultDataset)
						# print(faultRate)
						resultDataset <- rbind(resultDataset,faultRate)
		...
		resultDataset <- colMeans(resultDataset)
		resultDataset <- cbind(samplingVector,resultDataset)
		colnames(resultDataset) <- NULL
